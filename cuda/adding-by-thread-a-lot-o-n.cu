...
constexpr int N = 1 << 20; // 2 na 20 - moramo da imamo vise blokova sa vise tredova
...
// block_count = N/thread_count

// ( N+THREAD_COUNT-1 ) / THREAD_COUNT <= formula

__global__ void add_by_block(int *a, int *b, int *c, int max_size)
{
    int tid = blockIdx.x * blockDim.x + threadIdx.x; // kao kod dinamickih nizova
    while (tid < N)
    {
        c[tid] = a[tid] + b[tid];
...
int main(void)
{
    ...
    dim3 blocks_per_grid((N + THREAD_COUNT - 1) / THREAD_COUNT);
